# DSPlayer

既存 PeerCast 視聴環境向けの **PCRPlayer 代替プレイヤー**。

```
PeCaRecorder → DSPlayer.exe "$x" "$0" "$3"
```

PeerCastStation / YP / PeCaRecorder は変更しません。

## 必要環境

- Windows 10/11 **x64**
- .NET 8 Desktop Runtime（開発時は SDK 8）
- libmpv（`libmpv-2.dll` または `mpv-1.dll`）

## セットアップ

```powershell
cd "D:\Vive cording\DSPlayer"
.\scripts\fetch-libmpv.ps1
dotnet build .\src\DSPlayer\DSPlayer.csproj -c Release

dotnet run --project .\src\DSPlayer\DSPlayer.csproj -c Release -- `
  "http://localhost:7144/pls/<ChannelID>?tip=host:port" `
  "テストch" `
  "https://example.com/test/read.cgi/board/1/"
```

## PeCaRecorder への登録

| 項目 | 値 |
|------|-----|
| パス | `...\DSPlayer.exe` |
| 引数 | **`"$x" "$0" "$3"`** |
| タイプ | `FLV\|WMV` |

詳細は [docs/PECARECORDER.md](docs/PECARECORDER.md)。

## 操作

| 操作 | 動作 |
|------|------|
| マウスホイール | 音量 ±5 |
| Space | 一時停止 / 再開 |
| ↑ / ↓ | 音量 |
| F11 / ダブルクリック | フルスクリーン |
| Esc | 画像プレビューを閉じる / フルスクリーン解除 |
| 動画右上ホバー | 最小化・最大化・閉じる |
| 動画中央をドラッグ | ウィンドウ移動。画面端・タスクバー手前・他のウィンドウの端に吸着（設定でオフ可） |
| Shiftを押しながら移動 | ウィンドウの吸着を一時解除 |
| 境界ドラッグ | 動画/コメント幅 |
| コメント内 `>>N` | 表示中の該当レスへジャンプ |
| コメント内 URL | 既定ブラウザで開く |
| コメント内画像 | 欄内に表示。クリックで枠なしプレビュー |
| 画像プレビュー | 押しっぱなしで移動。クリック / Esc / 右クリックで閉じる。端リサイズは縦横比維持 |
| コメントを遡っているとき | 新着で最下部へ戻さない。下の「新着 N ↓」で最新へ |
| スレタイクリック | スレッド一覧 |
| 右クリック | メニュー（設定・掲示板など） |
| 右クリック → 再接続 | 現在のチャンネルに接続し直す。自動再接続の待機中も使用可 |
| 書込 Shift+Enter | レス書き込み |
| 書込 Enter | 改行（欄が下に伸びる） |
| フルスクリーン下端ホバー | 書込欄・ステータスをフロート表示（動画サイズは不変） |

設定: `%LOCALAPPDATA%\DSPlayer\settings.json`  
ログ: `%LOCALAPPDATA%\DSPlayer\player.log`

## 開発フェーズ

詳細は [docs/SPEC.md](docs/SPEC.md)。

1. ~~調査~~ 〜 6. ~~コメント UI / 窓クローム~~
7. ~~安定化・再接続・終了処理・差し替え運用~~ **完了**（2026-08-12）
8. ~~勢いメーター・コメント追従・本文リンク/画像~~ **完了**（2026-08-23）

### 次のステップ

- [docs/PECARECORDER.md](docs/PECARECORDER.md) に従い **本番差し替え・運用**
- 不具合は実運用ベースで対応
