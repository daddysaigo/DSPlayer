# PeCaRecorder 連携・差し替え手順

DSPlayer を **PCRPlayer の代替**として使うための手順です。  
PeerCastStation / YP / PeCaRecorder 本体の改変は不要です。

```
配信者 → OBS → PeerCastStation → YP → PeCaRecorder → DSPlayer
                                                      ↑ ここだけ置換
```

## 1. 推奨: プレイヤーとして追加登録

PeCaRecorder → **オプション → 全般の設定 → プレイヤー**

| 項目 | 値 |
|------|-----|
| 有効 | チェック |
| パス | 展開した配布ZIP内の`DSPlayer.exe` |
| 引数 | `"$x" "$0" "$3" "$6"` |
| タイプ | `FLV\|WMV` または `FLV\|UNKNOWN` |
| ブラウザで開く | オフ |
| 接続してから起動 | オフ（任意） |
| IDを検証 | オン推奨 |

### プレースホルダ（元 PCRPlayer と同じ）

| 記号 | 意味 | 例 |
|------|------|-----|
| `$x` | ストリーム / プレイリスト URL | `http://127.0.0.1:7144/pls/<ID>?tip=host:port` |
| `$0` | チャンネル名 | `木寺 (FLV)` |
| `$3` | Contact URL（掲示板） | `https://.../read.cgi/...` |
| `$6` | YPが通知したリスナー数 | `75` |

**必ず `$3` を含めてください。**  
これが無いとコメント欄の Contact 解決が PeerCast API 頼みになり、板が開かないことがあります。

元の `PeCaRecorder.xml` 登録例:

```xml
<item enable="true"
      path="PCRPlayer\PCRPlayer.exe"
      arg="&quot;$x&quot; &quot;$0&quot; &quot;$3&quot; &quot;$6&quot;"
      type="WMV|FLV"
      browser="false" connect="false" verify="true"/>
```

### 起動後の内部変換

| 入力 | 再生 URL |
|------|----------|
| `.../pls/<id>?tip=...` | `.../stream/<id>?tip=...`（`tip` は維持） |
| `.../stream/<id>...` | そのまま |

## 2. 差し替え運用（PCRPlayer.exe 名で置く）

最終的には **ファイル名を `PCRPlayer.exe` にして** 既存パスを上書きできます。

### 手順

1. 既存の PCRPlayer フォルダをバックアップ  
   例: `...\PeCaRecorder_v091\PCRPlayer` → `PCRPlayer.bak`
2. Release ビルド:

   ```powershell
   cd DSPlayer
   .\scripts\fetch-libmpv.ps1   # 未取得時
   dotnet build .\src\DSPlayer\DSPlayer.csproj -c Release
   ```

3. 出力フォルダの中身を用意:

   ```
   DSPlayer.exe   →  PCRPlayer.exe にリネーム
   lib\mpv 系 DLL     →  同じ階層の lib\ または exe 横（ビルド設定に従う）
   ```

4. 配布物を PCRPlayer フォルダへコピー（**上書き前にバックアップ必須**）
5. PeCaRecorder のプレイヤー設定パスが  
   `PCRPlayer\PCRPlayer.exe` のままなら **引数だけ**  
   `"$x" "$0" "$3" "$6"` になっているか確認
6. チャンネルを再生して、映像・コメント・書き込みを確認

### 注意

- DSPlayer は **x64** 専用（`PCRPlayer64.exe` 相当）
- 32bit のみの環境は非対応
- 設定は exe 横の XML ではなく  
  `%LOCALAPPDATA%\DSPlayer\settings.json`
- 本格 BBS UI は引き続き同梱の **PCRBrowser.exe** を起動可能  
  （右クリック → BBSブラウザ）

### 同梱したいもの（任意）

| 元フォルダ | 用途 |
|------------|------|
| `PCRBrowser.exe` | 本格掲示板ブラウザ |
| `skin\` | PCRBrowser 用（DSPlayer 本体は未使用） |

## 3. 手動起動テスト

```powershell
# ストリームのみ
dotnet run --project .\src\DSPlayer\DSPlayer.csproj -c Release -- `
  "http://127.0.0.1:7144/stream/<ChannelID>" "テストch"

# 本番相当（$x $0 $3 $6）
dotnet run --project .\src\DSPlayer\DSPlayer.csproj -c Release -- `
  "http://127.0.0.1:7144/pls/<ChannelID>?tip=host:port" `
  "チャンネル名" `
  "https://bbs.example/test/read.cgi/board/1234567890/" `
  "75"
```

## 4. 受け入れチェックリスト

### 起動・再生（Phase 5）

- [ ] PeCaRecorder からダブルクリックで起動する
- [ ] `/pls/?tip=` が映像として再生される
- [ ] 情報バーにチャンネル名が出る
- [ ] `$3` の Contact でコメントが流れる
- [ ] （差し替え時）`PCRPlayer.exe` 名でも起動する

### 安定化（Phase 7）— クローズ 2026-08-12

開発受け入れ完了。差し替え前後の再確認用に残す。

- [x] 配信を止めてもプレイヤーが落ちず「再接続」表示になる
- [x] 配信再開後に映像が戻る（自動）
- [x] 再接続中ステータス表示 / stall 監視
- [x] 動画右上ホバーで min/max/close
- [x] フルスクリーンで書込/ステータスがフロート（動画サイズ不変・切替スムーズ化）
- [x] コメント `>>N` ジャンプ
- [x] 設定フォント・レス情報 ON/OFF
- [x] ステータス kbps が実受信（約1秒）

### 勢い・コメント本文（Phase 8）— クローズ 2026-08-23

- [x] ステータスに勢い（最低帯は「過疎」）
- [x] コメントを遡っているとき新着で最下部へ引っ張られない
- [x] 遡り中は「新着 N ↓」で最新へ戻れる
- [x] 本文 URL クリックでブラウザ
- [x] 画像 URL がコメント幅内に表示される（Twitter / 透過 GIF 含む）
- [x] 画像クリックで枠なしプレビュー（ドラッグ移動、Esc / クリック / 右クリックで閉じる）
- [ ] （運用）長時間切断放置・連続 ch 切替の追加安心確認
- [ ] （運用）終了後プロセス残存なしの最終確認
- [ ] （運用）PeCaRecorder 本番登録 / `PCRPlayer.exe` 差し替え
## 5. トラブルシュート

| 症状 | 確認 |
|------|------|
| 黒画面 | PeerCastStation が受信中か、`/stream/` でブラウザ/curl できるか |
| コメント無し | 引数に `$3` があるか、Contact が BBS URL か |
| libmpv エラー | `lib\mpv-2.dll`（または `mpv-1.dll`）が出力先にあるか |
| 起動しない | x64 OS か、.NET 8 Desktop Runtime が入っているか |
| 起動直後に落ちる | `%LOCALAPPDATA%\DSPlayer\player.log` を確認 |
| 再接続し続ける | tip / チャンネル ID が生きているか、YP で配信中か |
| 切断後固まったまま | ログの `end-file` / `stall watchdog` / `schedule retry`。Release 最新ビルドか |
| メモリが本家より多い | .NET+WPF+mpv のため起動~140MB前後は想定内。無限増のみ要調査 |
| FS でバー表示時に動画が縮む | 古いビルドの可能性。フロート実装入りの exe か確認 |
| コメントが新着で一番下に飛ばされる | 最下部付近にいると追従する。少し上にスクロールすれば固定。「新着 N ↓」で戻る |
| 画像が出ない | 設定の「画像URLをコメント欄に表示」がオンか。jpg/png/gif/bmp か Twitter メディア URL か。WebP はリンクのみ |
| GIF が穴だらけ / 大きさがループで変わる | 合成入りの Release か。古いビルドは差分フレームをそのまま出していた |
| 画像プレビューが映像の上に出ない | 別ウィンドウ実装入りの exe か。オーバーレイは mpv HWND に隠れる |
